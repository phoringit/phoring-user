...
class WalletController extends ChangeNotifier {
  ...
  Future <void> addFundToWallet(String amount, String paymentMethod) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      showCustomSnackBar("Minimum= ${PriceConverter.convertPrice(Get.context!, apiResponse.response?.data['minimum_amount'] )} and Maximum=${PriceConverter.convertPrice(Get.context!, apiResponse.response?.data['maximum_amount'].toDouble())}" , Get.context!);
    }else{
      _isConvert = false;
      ApiChecker.checkApi(apiResponse);
    }
